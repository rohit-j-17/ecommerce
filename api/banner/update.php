...
function perform()
{
    ...
    if ($payload['operation'] == "UPDATE") {
        if (isset($payload['bannerId']) && isset($payload['sName']) && isset($payload['sDescription'])) {

            $banner = new BannerDatabase();
            return $banner->update($payload['bannerId'], $payload['sName'], $payload['sDescription'], $imgObj);
        } else {
