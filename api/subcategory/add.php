...
function perform()
{
    ...
    $subcategoryDB = new SubCategoryDatabase();

    if ($payload['operation'] == "ADD") {

        if(!(isset($payload['categoryId']) && isset($payload['sName']) && isset($payload['sDescription']) && isset($imgObj)))
         {
            return $response = array("status" => EResponseCode::INVALID_INPUT, "msg" => "Invalid input");
        }
        
        $response = $subcategoryDB->add($payload['categoryId'], $payload['sName'], $payload['sDescription'], $imgObj);
    } else {
